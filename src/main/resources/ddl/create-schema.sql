CREATE TABLE jpaforbeginners.public.tbl_persons (ID BIGINT NOT NULL, col_birthday DATE NOT NULL, col_dateinsert timestamp with time zone, col_dateupdate timestamp with time zone, col_deathdate DATE, col_firstname VARCHAR(150) NOT NULL, col_gender CHAR(1) NOT NULL, col_height numeric(3,2), col_weight numeric(4,2), PRIMARY KEY (ID))
CREATE TABLE jpaforbeginners.public.tbl_products (ID BIGINT NOT NULL, col_dateinsert timestamp with time zone, col_dateupdate timestamp with time zone, col_description VARCHAR(255) NOT NULL, col_discount NUMERIC(3,2), col_title VARCHAR(150) NOT NULL, col_unit SMALLINT NOT NULL, col_unitPrice DECIMAL(11,2) NOT NULL, col_validity DATE, PRIMARY KEY (ID))
...
CREATE SEQUENCE jpaforbeginners.public.products_id_seq INCREMENT BY 50 START WITH 199
