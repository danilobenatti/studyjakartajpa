CREATE TABLE jpaforbeginners.public.persons (ID BIGINT NOT NULL, peType char(1), birthdate DATE NOT NULL, date_insert TIMESTAMP WITH TIME ZONE, date_update TIMESTAMP WITH TIME ZONE, deathdate DATE, first_name VARCHAR(150) NOT NULL, gender CHAR(1) NOT NULL, height NUMERIC(3,2) default 0.0, weight NUMERIC(3,1) default 0.0, partner_id BIGINT, PRIMARY KEY (ID))
CREATE TABLE jpaforbeginners.public.products (ID BIGINT NOT NULL, active BOOLEAN, dateinsert TIMESTAMP WITH TIME ZONE, dateupdate TIMESTAMP WITH TIME ZONE, description VARCHAR(255) NOT NULL, discount DECIMAL(4,2) NOT NULL, title VARCHAR(150) NOT NULL, unit SMALLINT NOT NULL, unitPrice DECIMAL(18,2) NOT NULL, validity DATE, PRIMARY KEY (ID))
...
CREATE TABLE EMPLOYEE (ID BIGINT NOT NULL, hiringDate DATE NOT NULL, jobfunction SMALLINT NOT NULL, register VARCHAR(255) NOT NULL, salary DECIMAL(18,2) NOT NULL, PRIMARY KEY (ID))
CREATE TABLE jpaforbeginners.public.persons_emails (person_id BIGINT  /*  */ , email VARCHAR(150) NOT NULL)
CREATE TABLE jpaforbeginners.public.persons_phones (person_id BIGINT NOT NULL  /*  */ , phoneNumber VARCHAR(20) NOT NULL, phType char(1))
...
ALTER TABLE jpaforbeginners.public.persons ADD CONSTRAINT uk_persons_partner_id UNIQUE (partner_id)
...
ALTER TABLE EMPLOYEE ADD CONSTRAINT FK_EMPLOYEE_ID FOREIGN KEY (ID) REFERENCES jpaforbeginners.public.persons (ID)
